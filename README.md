...
RNA secondary structures were obtained from RSID, while RNA tertiary structures were generated using multiple structure prediction tools as described in the manuscript.
...
Each RNA tertiary structure should be provided as a PDB file. Under the default configuration, PDB files are stored as:

```text
data/3D/alphafold3/PDB/<RNA_ID>.pdb
```

Each filename must match the corresponding `RNA_ID` in `RNA.xlsx`.

The directory name `alphafold3` denotes the default structure source. Structures from other sources can follow the same layout:

```text
data/3D/<structure_source>/PDB/<RNA_ID>.pdb
```

In this study, RNA 3D structures generated by AlphaFold 3, RNAComposer, DRfold2, RhoFold+, SimRNA, and trRosettaRNA were evaluated. Please refer to the official implementations of these tools when generating predicted structures.
...
GeoM3-RMI uses BioBERT to encode RNA and small-molecule textual information.
The pretrained model is not redistributed with this repository. Users should obtain BioBERT from its official distribution and place the model and tokenizer files under:
...
The released training pipeline uses random stratified five-fold cross-validation.
For each fold, a validation set is sampled from the training portion, and the checkpoint with the highest validation AUC is retained for final evaluation. with the default warm-start pair split. The validation set is constructed from the training portion of each fold, and the checkpoint with the highest validation AUC is retained.
...
```text
model/3D_alphafold3_fold<fold>.pt
```
...
```text
result/result_3D_alphafold3_fold<fold>_<timestamp>.csv
```
